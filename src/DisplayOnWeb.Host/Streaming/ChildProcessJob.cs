...
public sealed class ChildProcessJob : IDisposable
{
    ...
    public void StopOrphanedDisplayOnWebProcesses()
    {
        ...
        foreach (ManagementObject item in searcher.Get())
        {
            ...
            if (IsLiveDisplayOnWebHost(parentId)) continue;
    ...
    private static bool IsLiveDisplayOnWebHost(int processId)
    {
        try
        {
            using var parent = Process.GetProcessById(processId);
            return parent.ProcessName.Equals("DisplayOnWeb.Host", StringComparison.OrdinalIgnoreCase) ||
                   ...
                   (parent.ProcessName.Equals("dotnet", StringComparison.OrdinalIgnoreCase) &&
                    ReadCommandLine(processId).Contains("DisplayOnWeb.Host", StringComparison.OrdinalIgnoreCase));
        }
